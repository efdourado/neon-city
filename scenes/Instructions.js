export default class Instructions extends Phaser.Scene {
  ...
  create() {
    ...
    const boxWidth = 860;
    const boxHeight = 430;
    const boxY = height / 2 - 6;

    this.add.rectangle(width / 2, boxY, boxWidth, boxHeight, 0x000000, 0.6)
      .setStrokeStyle(2, 0xaaaaaa, 0.3);

    const infoText = [
      '| Missão: Recuperar o Data-Core roubado',
      '',
      '| Fases:',
      '  [01] Telhados: buscar cartões e abrir a porta',
      '  [02] Laboratório: ativar switches e sobreviver',
    ...
    const terminalText = this.add.text(width / 2 - (boxWidth / 2) + paddingX, boxY - (boxHeight / 2) + paddingY, infoText, {
      ...
      color: '#ffffff',
      lineSpacing: 6,
      wordWrap: { width: boxWidth - (paddingX * 2), useAdvancedWrap: true }
    }).setOrigin(0, 0); 

    terminalText.setAlpha(0.92);
    // ---------------------------------------------------
